...
function App() {
  ...
  const checkCrypto = (coin) => {
    const apiURL = `http://localhost:5000/${coin}`
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <a onClick={() => checkCrypto('BTC')} className="App-link"> Bitcoin </a>
        <a onClick={() => checkCrypto('ETH')} className="App-link"> Ethereum </a>
        <a onClick={() => checkCrypto('XRP')} className="App-link"> Ripple </a>
        <section>
